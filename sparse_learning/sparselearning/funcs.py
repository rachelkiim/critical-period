...
def random_growth(masking, name, new_mask, total_regrowth, weight):
    ...
    new_weights = torch.rand(new_mask.shape, device=weight.device) < expeced_growth_probability
...
def gradient_growth(masking, name, new_mask, total_regrowth, weight):
    """RigL-style growth: grow weights where the (dense) gradient magnitude
    is largest, using the instantaneous gradient of the current backward
    pass rather than the optimizer's momentum buffer.
    """
    grad = weight.grad
    if grad.dtype == torch.float16:
        grad = grad*(new_mask==0).half()
