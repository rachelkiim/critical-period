...
class Masking(object):
        ...
        if mode == 'constant':
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    ...
                    self.masks[name][:] = (torch.rand(weight.shape) < density).float().data.to(weight.device)
            ...
            for name, weight in module.named_parameters():
                ...
                self.masks[name][:] = (torch.rand(weight.shape) < prob).float().data.to(weight.device)
            self.apply_mask()
    ...
    def add_module(self, module, density, sparse_init='constant', remove_param_names=()):
        ...
        for name, tensor in module.named_parameters():
            ...
            self.masks[name] = torch.zeros_like(tensor, dtype=torch.float32, requires_grad=False, device=tensor.device)
