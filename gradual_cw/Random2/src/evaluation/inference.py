...
def compute_epoch_metrics(
    ...
    """
    Compute all metrics for a single epoch evaluation.

    Args:
        model: The PyTorch model to evaluate.
        loader: DataLoader providing the data.
        device: Device to use for evaluation.
        num_bin: Number of bins for ECE calculation.

    Returns:
        dict: Dictionary containing loss, accuracy, ECE, and mean confidence.
    """
    from src.evaluation.calibration import reliability_diagram, ece

    pred_correct, conf, _, _ = get_predictions_and_confidence(model, loader, device)

    accuracy = np.mean(pred_correct)
    mean_conf = np.mean(conf)

    # Compute ECE
    acc_bins, conf_bins, num_samples = reliability_diagram(pred_correct, conf, num_bin)
    ece_value = ece(acc_bins, conf_bins, num_samples)
